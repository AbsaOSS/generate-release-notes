...
class DataMiner:
    ...
    def mine_missing_sub_issues(self, data: MinedData) -> tuple[dict[Issue, Repository], dict[str, list[PullRequest]]]:
        ...
        prs_of_fetched_cross_repo_issues = self._fetch_prs_for_fetched_cross_issues(fetched_issues, data)
    ...
    def _fetch_prs_for_fetched_cross_issues(self, issues: dict[Issue, Repository], data: MinedData) -> dict[str, list[PullRequest]]:
        ...
        for i, repo in issues.items():
            ...
            for ev in i.get_timeline():  # timeline includes cross-references
                if ev.event == "cross-referenced" and getattr(ev, "source", None):
                    src_issue = ev.source.issue  # <- this is a github.Issue.Issue
