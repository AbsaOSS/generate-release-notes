...
def mock_repo():
    mr = Mock(spec=Repository)
    mr.full_name = "mock/repo"
    return mr
...
def __get_default_issue_mock(number: int, state: str, labels: list[str] = None,
    ...
    if labels is None:
        ...
        labels_mocks = []
        for label in labels:
            mock_label = Mock(spec=MockLabel)
            mock_label.name = label
            labels_mocks.append(mock_label)
        mock_issue.labels = labels_mocks
...
def __get_record_mock_1_issue_with_2_prs(repository: Repository, issue_state: str = "closed",
                                         issue_labels: list[str] = None, pr_with_rls_notes: bool = True,
                                         second_pr_merged: bool = True,
                                         is_closed_not_planned:bool = False) -> dict[int, Record]:
    # Note: Opened issue are not fetched now in 1st wave
    #   They can be fetched on demand of detected mentioned in PR body and not present among fetched issues
    ...
    records = dict()
    records[0] = Record(repo=repository, issue=issue)
...
def __get_record_mock_2_issue_with_2_prs(repository: Repository,
                                         issue_1_state: str = "closed", issue_2_state: str = "closed",
    ...
    if issue_1_state == "closed":
        records[0] = Record(repo=repository, issue=issue1)
    else:
        records[0] = Record(repo=repository)
    if issue_2_state == "closed":
        records[1] = Record(repo=repository, issue=issue2)
    else:
        records[1] = Record(repo=repository)
    records[0].register_pull_request(pr1)
...
def __get_record_mock_1_pr_with_no_issue(repository: Repository, state: str = "closed", is_merged: bool = True,
                                         labels: list[str] = None, with_rls_notes: bool = True,
                                         is_draft: bool = False) -> dict[int, Record]:
    # create 2 PRs
...
- #1 _I1+0PR+1L-bug_ in [#101](https://github.com/mock/repo/pull/101), [#102](https://github.com/mock/repo/pull/102)
...
- #1 _I1+0PR+2L-bug-enhancement_ in [#101](https://github.com/mock/repo/pull/101), [#102](https://github.com/mock/repo/pull/102)
  - PR 101 1st release note
  - PR 101 2nd release note
  - PR 102 1st release note
  - PR 102 2nd release note
...
- #1 _I1+0PR+0L_ in [#101](https://github.com/mock/repo/pull/101), [#102](https://github.com/mock/repo/pull/102)
  - PR 101 1st release note
  - PR 101 2nd release note
  - PR 102 1st release note
  - PR 102 2nd release note
...
def test_release_notes_builder(test_case):
    ...
    # g = Mock(spec=Github) TODO delete it with import
    repository_mock = Mock(spec=Repository)
    repository_mock.full_name = 'mock/repo'
    ...
    print(f"Expected - {test_case['test_name']}:\n" + expected_release_notes)
    assert expected_release_notes == actual_release_notes
